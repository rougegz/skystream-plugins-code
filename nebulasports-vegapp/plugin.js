(function() {
  ...
  async function _httpJsonOnce(url, timeoutMs, depth) {
    ...
    if (resp && (s === 301 || s === 302 || s === 303 || s === 307 || s === 308) && depth < 3) {
      var loc = "";
      try {
        var h = resp.headers || {};
        var hk = Object.keys(h);
        for (var hi = 0; hi < hk.length; hi++) if (hk[hi].toLowerCase() === "location") loc = safeStr(h[hk[hi]]);
      } catch (e) {}
      if (loc && !/^https?:/i.test(loc) && loc.charAt(0) === "/") {
        var om = safeStr(url).match(/^(https?:\/\/[^/]+)/);
        if (om) loc = om[1] + loc;
      }
      if (/^https?:/i.test(loc)) return _httpJsonOnce(loc, timeoutMs, depth + 1);
    }
